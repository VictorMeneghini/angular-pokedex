export const BAD_REQUESTS_ERROR: any = {
  "auth/missing-email": "Please provide a valid email address",
  "auth/internal-error": "Please provide a valid password",
  "auth/user-not-found": "Sorry, we couldn't find any pokemon trainer with this email",
  "auth/wrong-password": "Oops, check if your password is correct and try again (:",
  "auth/invalid-email": "Please provide a valid email address",
  "generic-error": "Sorry but something went wrong, please check your credentials and try again"
}
